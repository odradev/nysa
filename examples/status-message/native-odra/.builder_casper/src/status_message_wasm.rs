...
#[no_mangle]
fn set_status() {
    let (mut _contract, _): (odra_example_status::StatusMessage, _) =
        odra::StaticInstance::instance(&KEYS);
    let message = odra::casper::casper_contract::contract_api::runtime::get_named_arg("message");
    _contract.set_status(message);
}
#[no_mangle]
fn get_status() {
    let (_contract, _): (odra_example_status::StatusMessage, _) =
        odra::StaticInstance::instance(&KEYS);
