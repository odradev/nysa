    ...
    if let Some(v) = &body {
        match v {
            pt::Statement::Block { loc, unchecked, statements } => {
                let stmts = statements.iter().map(stmt::parse_statement).collect::<Vec<_>>();
                syn::Block { brace_token: Default::default(), stmts }
            },
            _ => panic!("Invalid statement - pt::Statement::Block expected")
        }
    } else {
        syn::Block { brace_token: Default::default(), stmts: vec![] }
    }
}

pub fn parse_attrs_to_receiver_param(attrs: &Vec<pt::FunctionAttribute>) -> Option<syn::FnArg> {
    if let Some(attr) = attrs.iter().find(|attr| matches!(attr, pt::FunctionAttribute::Mutability(_))) {
        match attr {
...
pub fn parse_parameter(param: &pt::Parameter) -> syn::FnArg {
    ...
    let name = param.name
        .as_ref()
        .and_then(|id| Some(id.name.clone())).unwrap_or_default();
    let name = format_ident!("{}", name);
    parse_quote!( #name: #ty )
}

pub fn parse_attrs(attrs: &Vec<pt::FunctionAttribute>) -> Vec<syn::Attribute> {
    attrs
        .iter()
...
pub fn parse_ret_type(func: &pt::FunctionDefinition) -> syn::ReturnType {
    if func.return_not_returns.is_some() {
        ...
        if returns.is_empty() {
            ...
            let types: syn::punctuated::Punctuated<syn::Type, syn::Token![,]> = returns.iter()
                .map(|ret| parse_type_from_expr(&ret.1.as_ref().unwrap().ty) )
                .collect();
            let tuple = syn::TypeTuple { paren_token: Default::default(), elems: types };
            syn::ReturnType::Type(Default::default(), Box::new(syn::Type::Tuple(tuple))) // return tuple
        }
    }
}
