...
// use super::{
//     num, parse,
//     syn_utils::{ArrayReader, DefaultValue, ReadValue, UnwrapOrNone, UnwrapOrRevert},
// };
// use crate::{
//     error::ParserResult,
...
//         context::{self, FnContext, ItemType, StorageInfo, TypeInfo},
//         odra::expr::array,
//         syn_utils::{AsExpression, AsSelfField, in_context},
//     },
...
// pub fn get_var_or_parse<T: StatementParserContext>(
//     expr: &Expression,
//     ctx: &mut T,
...
// /// * `ctx` - parser Context.
// pub fn assign<T: StatementParserContext, O: Into<BinOp> + Clone>(
//     left: &Expression,
...
// fn assign_default<T: StatementParserContext>(
//     left: &Expression,
//     ctx: &mut T,
// ) -> ParserResult<syn::Expr> {
...
//                 .map(|k| parse_storage_key(&k, ctx))
//                 .collect::<ParserResult<Punctuated<syn::Expr, Token![,]>>>()?;
//             dbg!(keys.to_token_stream().to_string());
//             parse_quote!((#keys))
//             // keys.as_expression()
//         }
//     };
